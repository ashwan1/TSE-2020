...
def _get_selected_text_for_xlnet(text, a, b, tokenizer):
    encoded_text = tokenizer.encode(text, add_special_tokens=False)
    pred_selected_text = tokenizer.decode(encoded_text[a: b + 1])
    return pred_selected_text


def get_jaccard_from_df(df: pd.DataFrame, start_idx: np.ndarray, end_idx: np.ndarray, model_type: str) -> float:
    ...
    df.to_csv('xlnet_results.csv', index=False)
